...
abstract class DataOperation {
    ...
    int deleteDataLowMemory(Uri uri) {
        if (belowMemThreshold()) {
            SlLog.i(TAG, "There is not enough space left on the device to store events, so will delete 100 oldest events");
    ...
    private long getMaxCacheSize(Context context) {
        try {
            return SlDataAPI.sharedInstance(context).getMaxCacheSize();
        } catch (Exception e) {
