...
class Logger {
    ...
    static void printCopyright() {
        ...
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###                                                              ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###          欢迎使用 SlAnalytics® (v" + SlAnalyticsTransform.VERSION + ")编译插件         ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###               使用过程中碰到任何问题请联系我们                     ###" + "${LogUI.E_NORMAL.value}")
        println("${LogUI.C_BLACK_GREEN.value}" + "###                      QQ 号：3091808044                        ###" + "${LogUI.E_NORMAL.value}")
...
//        println("${LogUI.C_BLACK_GREEN.value}" + "###     https://github.com/sensorsdata/sa-sdk-android-plugin2    ###" + "${LogUI.E_NORMAL.value}")
    ...
    def static error(Object msg) {
        try {
            println("${LogUI.C_ERROR.value}[SlAnalytics]: ${msg}${LogUI.E_NORMAL.value}")
        } catch (Exception e) {
            e.printStackTrace()
        }
    }
    ...
    def static warn(Object msg) {
        try {
            println("${LogUI.C_WARN.value}[SlAnalytics]: ${msg}${LogUI.E_NORMAL.value}")
        } catch (Exception e) {
            e.printStackTrace()
        }
    }
