...
class SlAnalyticsClassVisitor extends ClassVisitor {
    ...
    private ArrayList<SlAnalyticsMethodCell> methodCells = new ArrayList<>()

    private int version

    private HashMap<String, SlAnalyticsMethodCell> mLambdaMethodCells = new HashMap<>()
    ...
    SlAnalyticsClassVisitor(final ClassVisitor classVisitor, ClassNameAnalytics classNameAnalytics, SlAnalyticsTransformHelper transformHelper) {
        super(SlAnalyticsUtil.ASM_VERSION, classVisitor)
    ...
    void visitEnd() {
        ...
        if (SlAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
            MethodVisitor mv
            // 添加剩下的方法，确保super.onHiddenChanged(hidden);等先被调用
            Iterator<Map.Entry<String, SlAnalyticsMethodCell>> iterator = SlAnalyticsHookConfig.FRAGMENT_METHODS.entrySet().iterator()
            while (iterator.hasNext()) {
                Map.Entry<String, SlAnalyticsMethodCell> entry = iterator.next()
                String key = entry.getKey()
                SlAnalyticsMethodCell methodCell = entry.getValue()
                ...
                visitMethodWithLoadedParams(mv, Opcodes.INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, methodCell.agentName, methodCell.agentDesc, methodCell.paramsStart, methodCell.paramsCount, methodCell.opcodes)
                mv.visitInsn(Opcodes.RETURN)
                mv.visitMaxs(methodCell.paramsCount, methodCell.paramsCount)
                mv.visitEnd()
                mv.visitAnnotation("Lbaseandroid/sl/sdk/analytics/SlDataInstrumented;", false)
            }
        }

        for (cell in methodCells) {
            transformHelper.slAnalyticsHookConfig."${cell.agentName}"(classVisitor, cell)
        }
        if (Logger.debug) {
    ...
    FieldVisitor visitField(int access, String name, String descriptor, String signature, Object value) {
        if (classNameAnalytics.isSlDataAPI) {
            if ('VERSION' == name) {
                String version = (String) value
                if (SlAnalyticsUtil.compareVersion(SlAnalyticsTransform.MIN_SDK_VERSION, version) > 0) {
                    String errMessage = "你目前集成的埋点 SDK 版本号为 v${version}，请升级到 v${SlAnalyticsTransform.MIN_SDK_VERSION} 及以上的版本。详情请参考：https://github.com/sensorsdata/sa-sdk-android"
                ...
                if (minPluginVersion != "" && minPluginVersion != null) {
                    if (SlAnalyticsUtil.compareVersion(SlAnalyticsTransform.VERSION, minPluginVersion) < 0) {
                        String errMessage = "你目前集成的插件版本号为 v${SlAnalyticsTransform.VERSION}，请升级到 v${minPluginVersion} 及以上的版本。详情请参考：https://github.com/sensorsdata/sa-sdk-android-plugin2"
                        Logger.error(errMessage)
                        throw new Error(errMessage)
                    }
                }
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if (transformHelper.extension != null && transformHelper.extension.autoHandleWebView && transformHelper.urlClassLoader != null) {
            methodVisitor = new SlAnalyticsWebViewMethodVisitor(methodVisitor, transformHelper, mClassName, mSuperName)
        }
        SlAnalyticsDefaultMethodVisitor slAnalyticsDefaultMethodVisitor = new SlAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            boolean isSlDataTrackViewOnClickAnnotation = false
            boolean isSlDataIgnoreTrackOnClick = false
            ...
            void visitEnd() {
                ...
                if (isHasTracked) {
                    ...
                    visitAnnotation("Lbaseandroid/sl/sdk/analytics/SlDataInstrumented;", false)
            ...
            void visitInvokeDynamicInsn(String name1, String desc1, Handle bsm, Object... bsmArgs) {
                ...
                try {
                    ...
                    SlAnalyticsMethodCell slAnalyticsMethodCell = SlAnalyticsHookConfig.LAMBDA_METHODS.get(Type.getReturnType(desc1).getDescriptor() + name1 + desc2)
                    if (slAnalyticsMethodCell != null) {
                        Handle it = (Handle) bsmArgs[1]
                        mLambdaMethodCells.put(it.name + it.desc, slAnalyticsMethodCell)
                    }
                } catch (Exception e) {
                    e.printStackTrace()
                }
            ...
            protected void onMethodEnter() {
                super.onMethodEnter()
                nameDesc = name + desc
                pubAndNoStaticAccess = SlAnalyticsUtil.isPublic(access) && !SlAnalyticsUtil.isStatic(access)
                if ((nameDesc == 'onClick(Landroid/view/View;)V') && pubAndNoStaticAccess) {
                    ...
                    methodVisitor.visitVarInsn(ISTORE, secondLocalId)
                    localIds.add(secondLocalId)
                } else if (SlAnalyticsUtil.isTargetMenuMethodDesc(nameDesc) && pubAndNoStaticAccess) {
                    localIds = new ArrayList<>()
            ...
            void handleCode() {
                if (isHasInstrumented || classNameAnalytics.isSlDataAPI) {
                    return
                }
                /**
                 * Fragment
                 ...
                 */
                if (SlAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
                    SlAnalyticsMethodCell slAnalyticsMethodCell = SlAnalyticsHookConfig.FRAGMENT_METHODS.get(nameDesc)
                    if (slAnalyticsMethodCell != null) {
                        visitedFragMethods.add(nameDesc)
                        if (isSetUserVisibleHint) {
                            methodVisitor.visitVarInsn(ALOAD, 0)
                            methodVisitor.visitVarInsn(ILOAD, variableID)
                            methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, slAnalyticsMethodCell.agentName, slAnalyticsMethodCell.agentDesc, false)
                            ...
                            visitMethodWithLoadedParams(methodVisitor, Opcodes.INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, slAnalyticsMethodCell.agentName, slAnalyticsMethodCell.agentDesc, slAnalyticsMethodCell.paramsStart, slAnalyticsMethodCell.paramsCount, slAnalyticsMethodCell.opcodes)
                        }
                        isHasTracked = true
                        return
                    }
                 ...
                 */
                if (transformHelper.extension.lambdaEnabled) {
                    SlAnalyticsMethodCell lambdaMethodCell = mLambdaMethodCells.get(nameDesc)
                    if (lambdaMethodCell != null) {
                        ...
                        boolean isStaticMethod = SlAnalyticsUtil.isStatic(access)
                        if (!isStaticMethod) {
                            if (lambdaMethodCell.desc == '(Landroid/view/MenuItem;)Z') {
                                ...
                                methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, lambdaMethodCell.agentName, '(Ljava/lang/Object;Landroid/view/MenuItem;)V', false)
                                isHasTracked = true
                                return
                            }
                        }
                        ...
                        methodVisitor.visitMethodInsn(Opcodes.INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, lambdaMethodCell.agentName, lambdaMethodCell.agentDesc, false)
                        isHasTracked = true
                        return
                    }
                }
                 ...
                 */
                if (SlAnalyticsUtil.isTargetMenuMethodDesc(nameDesc)) {
                    methodVisitor.visitVarInsn(ALOAD, localIds[0])
                    methodVisitor.visitVarInsn(ALOAD, localIds[1])
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackMenuItem", "(Ljava/lang/Object;Landroid/view/MenuItem;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (nameDesc == 'onDrawerOpened(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackDrawerOpened", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                } else if (nameDesc == 'onDrawerClosed(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackDrawerClosed", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (!SlAnalyticsUtil.isTargetClassInSpecial(mClassName)) {
                ...
                if (nameDesc == 'onItemSelected(Landroid/widget/AdapterView;Landroid/view/View;IJ)V' || nameDesc == "onListItemClick(Landroid/widget/ListView;Landroid/view/View;IJ)V") {
                    ...
                    methodVisitor.visitVarInsn(ALOAD, 2)
                    methodVisitor.visitVarInsn(ILOAD, 3)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackListView", "(Landroid/widget/AdapterView;Landroid/view/View;I)V", false)
                    isHasTracked = true
                    return
                }

                if (isSlDataTrackViewOnClickAnnotation && desc == '(Landroid/view/View;)V') {
                    trackViewOnClick(methodVisitor, 1)
                    isHasTracked = true
                    return
                }
                ...
                if (eventName != null && eventName.length() != 0) {
                    ...
                    methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "track", "(Ljava/lang/String;Ljava/lang/String;)V", false)
                    isHasTracked = true
                    return
                }
                ...
                if (mInterfaces != null && mInterfaces.length > 0) {
                    if (isOnItemClickMethod && mInterfaces.contains('android/widget/AdapterView$OnItemClickListener')) {
                        ...
                        SlAnalyticsMethodCell slAnalyticsMethodCell = SlAnalyticsHookConfig.INTERFACE_METHODS
                        ...
                        if (slAnalyticsMethodCell != null) {
                            ...
                            methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, slAnalyticsMethodCell.agentName, slAnalyticsMethodCell.agentDesc, false)
                            isHasTracked = true
                            return
                        }
                        ...
                        for (interfaceName in mInterfaces) {
                            SlAnalyticsMethodCell slAnalyticsMethodCell = SlAnalyticsHookConfig.INTERFACE_METHODS.get(interfaceName + nameDesc)
                            if (slAnalyticsMethodCell != null) {
                                visitMethodWithLoadedParams(methodVisitor, INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, slAnalyticsMethodCell.agentName, slAnalyticsMethodCell.agentDesc, slAnalyticsMethodCell.paramsStart, slAnalyticsMethodCell.paramsCount, slAnalyticsMethodCell.opcodes)
                                isHasTracked = true
                                return
                            }
                        }
                ...
                //同一个方法设置多个OnClicklisterer有问题，所以加个
                isHasTracked = false
            ...
            void handleClassMethod(String className, String nameDesc) {
                SlAnalyticsMethodCell slAnalyticsMethodCell = SlAnalyticsHookConfig.CLASS_METHODS.get(className + nameDesc)
                if (slAnalyticsMethodCell != null) {
                    visitMethodWithLoadedParams(methodVisitor, INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, slAnalyticsMethodCell.agentName, slAnalyticsMethodCell.agentDesc, slAnalyticsMethodCell.paramsStart, slAnalyticsMethodCell.paramsCount, slAnalyticsMethodCell.opcodes)
                    isHasTracked = true
                }
            ...
            boolean handleRN() {//sl没有
                boolean result = false
//                switch (transformHelper.rnState) {
//                    case SlAnalyticsTransformHelper.RN_STATE.NOT_FOUND:
//                        break
//                    case SlAnalyticsTransformHelper.RN_STATE.HAS_VERSION:
...
//                        }
//                        break
//                    case SlAnalyticsTransformHelper.RN_STATE.NO_VERSION:
//                        if (nameDesc == 'setJSResponder(IIZ)V' && mClassName == 'com/facebook/react/uimanager/NativeViewHierarchyManager') {
//                            methodVisitor.visitVarInsn(ALOAD, 0)
//                            methodVisitor.visitVarInsn(ILOAD, 1)
//                            methodVisitor.visitVarInsn(ILOAD, 2)
//                            methodVisitor.visitVarInsn(ILOAD, 3)
//                            methodVisitor.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackRN", "(Ljava/lang/Object;IIZ)V", false)
//                            result = true
//                        }
//                        break
//                }
            ...
            void trackViewOnClick(MethodVisitor mv, int index) {
                ...
                mv.visitMethodInsn(INVOKESTATIC, SlAnalyticsHookConfig.SL_ANALYTICS_API, "trackViewOnClick", "(Landroid/view/View;)V", false)
            }
...
//                if(s.contains("baseandroid/sl/sdk/analytics"))println("s="+s)
                if (s == 'Lbaseandroid/sl/sdk/analytics/SlDataTrackViewOnClick;') {
                    isSlDataTrackViewOnClickAnnotation = true
                    Logger.info("发现 ${name}${desc} 有注解 @SlDataTrackViewOnClick")
                } else if (s == 'Lbaseandroid/sl/sdk/analytics/SlDataIgnoreTrackOnClick;') {
                    isSlDataIgnoreTrackOnClick = true
                    Logger.info("发现 ${name}${desc} 有注解 @SlDataIgnoreTrackOnClick")
                } else if (s == 'Lbaseandroid/sl/sdk/analytics/SlDataInstrumented;') {
                    isHasInstrumented = true
                } else if (s == 'Lbaseandroid/sl/sdk/analytics/SlDataTrackEvent;') {
                    return new AnnotationVisitor(SlAnalyticsUtil.ASM_VERSION) {
                        @Override
        ...
        if (version <= Opcodes.V1_5) {
            return new SlAnalyticsJSRAdapter(SlAnalyticsUtil.ASM_VERSION, slAnalyticsDefaultMethodVisitor, access, name, desc, signature, exceptions)
        }
        return slAnalyticsDefaultMethodVisitor
    }
