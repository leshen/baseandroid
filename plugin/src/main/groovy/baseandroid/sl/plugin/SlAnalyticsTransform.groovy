...
class SlAnalyticsTransform extends Transform {
    ...
    SlAnalyticsTransform(SlAnalyticsTransformHelper transformHelper) {
        this.transformHelper = transformHelper
        if (!transformHelper.disableSlAnalyticsMultiThread) {
    ...
    boolean isIncremental() {
        return !transformHelper.disableSlAnalyticsIncremental
    }
    ...
    private void transformClass(Context context, Collection<TransformInput> inputs, TransformOutputProvider outputProvider, boolean isIncremental)
        ...
        println("[SlAnalytics]: 此次编译共耗时:${System.currentTimeMillis() - startTime}毫秒")
    }
    ...
    private void beforeTransform(TransformInvocation transformInvocation) {
        ...
        println("[SlAnalytics]: 是否开启多线程编译:${!transformHelper.disableSlAnalyticsMultiThread}")
        println("[SlAnalytics]: 是否开启增量编译:${!transformHelper.disableSlAnalyticsIncremental}")
        println("[SlAnalytics]: 此次是否增量编译:$transformInvocation.incremental")
        println("[SlAnalytics]: 是否在方法进入时插入代码:${transformHelper.isHookOnMethodEnter}")
    ...
    private void checkRNState() {
        //sl没有
//        try {
//            Class rnClazz = urlClassLoader.loadClass("com.sensorsdata.analytics.RNSlAnalyticsPackage")
//            try {
//                Field versionField = rnClazz.getDeclaredField("VERSION")
//                versionField.setAccessible(true)
//                transformHelper.rnVersion = versionField.get(null) as String
//                transformHelper.rnState = SlAnalyticsTransformHelper.RN_STATE.HAS_VERSION
//            } catch (Exception e) {
//                transformHelper.rnState = SlAnalyticsTransformHelper.RN_STATE.NO_VERSION
//            }
//        } catch (Exception e) {
            transformHelper.rnState = SlAnalyticsTransformHelper.RN_STATE.NOT_FOUND
//        }
    }
    ...
    void transformJar(File dest, JarInput jarInput, Context context) {
        ...
        if (!transformHelper.extension.disableJar || jarInput.file.absolutePath.contains('SlAnalyticsSDK')) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (entryName.endsWith(".DSA") || entryName.endsWith(".SF")) {
                ...
                try {
                    ...
                    sourceClassBytes = SlAnalyticsUtil.toByteArrayAndAutoCloseStream(inputStream)
                } catch (Exception e) {
    ...
    private byte[] modifyClass(byte[] srcClass, ClassNameAnalytics classNameAnalytics) {
        try {
            ...
            ClassVisitor classVisitor = new SlAnalyticsClassVisitor(classWriter, classNameAnalytics, transformHelper)
    ...
    private File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            ...
            ClassNameAnalytics classNameAnalytics = transformHelper.analytics(className)
            if (classNameAnalytics.isShouldModify) {
                byte[] sourceClassBytes = SlAnalyticsUtil.toByteArrayAndAutoCloseStream(new FileInputStream(classFile))
